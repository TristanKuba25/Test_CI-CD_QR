...
const cv::Scalar COLOR_YEL(0,255,255), COLOR_GRN(0,255,0),
                 COLOR_AIM(0,0,0);
...
double cpu_pct()
{
    ...
    double pct = (tot!=prev_tot)
        ? 100.0*double((tot-prev_tot)-(idle-prev_idle))/double(tot-prev_tot)
        : 0.0;
...
struct VxDelegate {
    ...
    explicit VxDelegate(const std::string& lib)
    {
        ...
        auto create =
            reinterpret_cast<Create>(dlsym(handle,"tflite_plugin_create_delegate"));
        destroy = reinterpret_cast<Destroy>(
            dlsym(handle,"tflite_plugin_destroy_delegate"));
        if(!create||!destroy) throw std::runtime_error("symbols missing");

        static const char* keys[]   = {};
        static const char* values[] = {};
        ptr = create(keys, values, 0);           // tableaux vides, size 0
...
cv::Mat binarize_image(const cv::Mat& img)
{
    ...
    cv::adaptiveThreshold(g,bin,255,
...
{
    std::memcpy(interpreter->typed_tensor<uint8_t>(input_index),
                inp.data,INPUT_W*INPUT_H*3);
    interpreter->Invoke();
...
inline void init_error_dir()
{
    ...
    if (ec)
        std::cerr << "[WARN] cannot create " << ERR_DIR
                  << " : " << ec.message() << '\n';
}
...
void save_error(const cv::Mat& img,
                const std::string& prefix,
                const ZXing::Error& err)
{
    char fname[256];
   /* << ZXing::ToString(err.type()) << ' '           
                        << err.msg() <<*/

    std::snprintf(fname, sizeof(fname), "%s%s_%s_%04d.jpg",
                  ERR_DIR.c_str(), prefix.c_str(),
...
cv::Mat run_odt_and_draw_results_frame(cv::Mat frame)
{
    ...
    auto objs = detect_objects(frame,DETECTION_THRESHOLD); //Reessayer avec inp apres
    ...
    for(const auto& o:objs){
       ...
       /* cv::putText(frame,cv::format("%.1f%%",o.at("score")*100.0f),
        ...
        cv::Mat roi_gray;
        cv::cvtColor(frame(cv::Rect(xmin,ymin,xmax-xmin,ymax-ymin)),
                     roi_gray,cv::COLOR_BGR2GRAY);

        ZXing::ReaderOptions opts;
        opts.setFormats(FORMAT_MASK)
        .setTryDownscale(true)
        .setReturnErrors(true);   

        /*save_error(roi_gray, "test_bf_decode", ZXing::Error::Format);
         printf("1 image avant decode saved\n");*/

        double tbegin=cv::getTickCount();

        auto res = ZXing::ReadBarcodes(
            ZXing::ImageView(roi_gray.data,roi_gray.cols,roi_gray.rows,
                             ZXing::ImageFormat::Lum,roi_gray.step),
            opts);
        ...
        for(const auto& r:res){
            ...
            /*save_error(roi_gray, "test", ZXing::Error::Format);
            printf("1 image test saved\n");*/

            if (r.isValid()){
                /*save_error(roi_gray, "valid", ZXing::Error::Format);
                printf("1 image valid saved\n");*/
                ...
                if (!r.text().empty()) {
                    ...
                    /*cv::putText(frame,r.text(),pts[0],
                                cv::FONT_HERSHEY_SIMPLEX,0.7,COLOR_GRN,2);*/
                ...
                //printf("erreur decoding\n");
                const ZXing::Error& err = r.error();

                /*std::cerr << "[ERR] "
                        << ZXing::ToString(err.type()) << ' '           
                        << err.msg() << " ("
                        << ZXing::ToString(r.format()) << ")\n";*/
...
int main(int argc,char* argv[])
{
    ...
    if(use_npu){
        try{
            ...
            if(interpreter->ModifyGraphWithDelegate(vx->ptr)==kTfLiteOk){
                std::cout<<"[INFO] NPU delegate active\n";
                vx_ok=true;
            }else{
                std::cerr<<"[WARN] VX refused, fallback CPU\n";
                vx.reset();
            }
        }catch(const std::exception& e){
            std::cerr<<"[WARN] VX init failed ("<<e.what()<<"), CPU mode\n";
            vx.reset();
        }
    }
    ...
    if(!vx_ok){
        ...
        if(interpreter->ModifyGraphWithDelegate(xnn)==kTfLiteOk)
            std::cout<<"[INFO] XNNPACK active ("<<num_threads<<" thr)\n";
        else {
            std::cerr<<"[FATAL] no delegate\n"; return 1;
        }
        interpreter->SetNumThreads(num_threads);
    } else {
        interpreter->SetNumThreads(1);          // thread léger côté CPU
    }
    ...
    while(true){
        ...
        // Target aim overlay
        cv::putText(frame,"Fit your Qr code in",
                    {frame.cols/2-85,frame.rows/2-10},
                    cv::FONT_HERSHEY_SIMPLEX,0.7,COLOR_AIM,2);
        ...
        if(fts.size()==AVG){
            ...
            std::cout<<std::fixed<<std::setprecision(1)
                     <<"[INFO] Mean FPS (each "<<AVG<<" frames) : "<<fps<<" | CPU "<<c<<"%\n";
            fts.clear(); cpu.clear();
        }
